...
public class SwagLabsTests {
    ...
    public void verifySuccessfulLogin () throws InterruptedException {
        ...
        wait    .withMessage("Button Logout is not visible.")
    ...
    public void productCheckout () {
        ...
        Assert.assertEquals(driver.findElement(By.className("inventory_item_name")).getText(),"Sauce Labs Backpack", "Sauce Labs item is not in the cart.");
    ...
    public void validateSocialLinksInFooter () throws InterruptedException, IOException {
        ...
        String twitterUrl = driver.findElement(By.cssSelector(".social .social_twitter a")).getAttribute("href");
        String facebookUrl = driver.findElement(By.cssSelector(".social .social_facebook a")).getAttribute("Href");
        String linkendinUrl = driver.findElement(By.cssSelector(".social .social_linkedin a")).getAttribute("href");

        int statusCodeTwitter = Helper.getHTTPResponseStatusCode(twitterUrl);
        int statusCodeFacebook = Helper.getHTTPResponseStatusCode(facebookUrl);
        int statusCodeLinkendin = Helper.getHTTPResponseStatusCode(linkendinUrl);

        Assert.assertEquals(statusCodeTwitter, 200, "Status code for Twitter is not 200.");
        Assert.assertEquals(statusCodeFacebook, 200, "Status code for Facebook is not 200.");
        Assert.assertEquals(statusCodeLinkendin, 200, "Status code for Linkendin is not 200.");
    }
    @Test (priority = 11, retryAnalyzer = SwagLabsRetry.class)
    public void testDefaultNameSort () throws InterruptedException {
        String username = "standard_user";
        String password = "secret_sauce";
        ...
        List<String> names = nameList.stream().map(n -> n.getText()).collect(Collectors.toList());
        List<String> sortedNames = names;
        Collections.sort(sortedNames);
        Collections.sort(sortedNames);

        for (int i = 0; i < names.size(); i++) {
            Assert.assertEquals(names.get(i), sortedNames.get(i), "Product names are not sorted alphabetically(A-Z)");
        }
    }
    @Test (priority = 12, retryAnalyzer = SwagLabsRetry.class)
    public void testDefaultNameSortReverse () throws InterruptedException {
        String username = "standard_user";
        String password = "secret_sauce";
        ...
        for (int i = 0; i < names.size(); i++) {
            Assert.assertEquals(names.get(i), sortedNames.get(i), "Product names are not sorted alphabetically(Z-A)");
        }
    }
    ...
    public void testSortPriceLowHigh () {
        ...
        List<String> price = priceList.stream().map(n -> n.getText()).collect(Collectors.toList());
        List<String> sortedPrice = price;
        Collections.sort(sortedPrice);

        for (int i = 0; i < price.size(); i++) {
            Assert.assertEquals(price.get(i), sortedPrice.get(i), "Products are not sorted by price in ascending order");
        }
    }
    @AfterClass
